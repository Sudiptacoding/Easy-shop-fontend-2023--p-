...
const ProductList = () => {
    ...
    const [moredata, setMoreDetails] = useState({})
    ...
    const handelDelet = (item) => {
        const checklike = likehandel.filter(l => l._id !== item._id)
        setLikeHandel(checklike)
        const checkcard = cardhandel.filter(l => l._id !== item._id)
        setCardHandel(checkcard)

        removeCard(item._id)
        removeCardData(item._id)

        fetch(`https://easy-ruby-macaw-cape.cyclic.app/delet/${item._id}`, {
            method: 'DELETE',
        });
        swal("Good job!", "Delet sucessfully please relode", "success");
        const filterValue = data.filter(card => card._id !== item._id)
        setData(filterValue)
    }
                                    ...
                                    return <tbody key={i} className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                        <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            ...
                                            <td onClick={() => handelView(item)} className="px-6 py-4 cursor-pointer">
                                                <a className="font-medium text-blue-600 cursor-pointer dark:text-blue-500 hover:underline">prevew</a>
                                            </td>
                                            <td onClick={() => handelDelet(item)} className="px-6 py-4 cursor-pointer">
                                                <a className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Delet Product</a>
                                            </td>
                                        </tr>
                                    </tbody>
                                })
